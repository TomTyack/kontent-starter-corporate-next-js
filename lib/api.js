...
function replacer(key, value) {
  if (key.startsWith("_") || key === "rawData") {
    return undefined;
  }
  return value;
}
...
export async function getPageStaticPropsForPath(params, preview = false) {
  ...
  if (isLandingPage) {
    ...
    result.related[listingPage.system.codename] = JSON.parse(JSON.stringify(linkedItems, replacer));  // TODO polish serialization of the js SDK response
    console.log(new Date(), "Page [[...slug]].js load listing page items start: ", params);
  }
